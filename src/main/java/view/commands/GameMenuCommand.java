...
public enum GameMenuCommand {
    ...
    SET_FOOD_RATE("\\s*set\\s+food\\s+rate\\s+-r\\s+(?<rate>-?\\d+)\\s*"),
    SET_TAX("\\s*set\\s+tax\\s+rate\\s+-r\\s+(?<rate>-?\\d+)\\s*"),
    SHOW_TAX_RATE("\\s*show\\s*tax\\s+rate\\s*"),
    DROP_BUILDING("\\s*drop\\s*building\\s+-x\\s+(?<x>\\d+)\\s+-y\\s+(?<y>\\d+)" +
            "\\s+-t\\s+(?<type>(\\S+|\"[^\"]*\"))\\s*"),
    ...
    CREATE_UNIT("\\s*create\\s+unit\\s+-t\\s+(?<type>(\\S+|\"[^\"]*\"))" +
            "\\s+-c\\s+(?<count>\\d+)\\s+-x (?<x>\\d+) -y (?<y>\\d+)\\s*"),
    REPAIR("\\s*repair\\s*"),
